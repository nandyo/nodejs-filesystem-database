...
export function selectFROM(data: string | string[], callback: Function){
    ...
    db.get(query, data, (err, result)=>{
        if(err){ callback(err) }
        else{  return penampungResult = {id : result.id, nama : result.name, kelas : result.classroom}; }
    });
...
export function selectALL(data: string | string[] | [string], callback: Function){
    const db = new Database("./db/siswa.db");
    let penampungResult = [];
    ...
    db.each(query, [data[0], data[1], data[3], data[4]], (err, result)=>{
        if(err){ console.log("aaaaaaaa"+data); console.log(err.message); }
        else {
